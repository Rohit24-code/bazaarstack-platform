...
export const getCustomerHome = asyncHandler(
  async (_req: Request, res: Response) => {
    ...
    const [banners, categories, recentProducts, promos, featuredProduct] =
      await Promise.all([
    ...
    res.json(
      ok({
        ...
        featuredProduct: featuredProduct.map((featuredProductItem) => {
          const image =
            featuredProductItem.images.find((item) => item.isCover)?.url ||
            featuredProductItem.images[0]?.url ||
            "";

          const finalPrice = featuredProductItem.salePercentage
            ? Math.round(
                featuredProductItem.price -
                  (featuredProductItem.price *
                    featuredProductItem.salePercentage) /
                    100,
              )
            : featuredProductItem.price;

          return {
            _id: String(featuredProductItem._id),
            title: featuredProductItem.title,
            brand: featuredProductItem.brand,
            image,
            price: featuredProductItem.price,
            finalPrice,
            salePercentage: featuredProductItem.salePercentage,
            createAt: featuredProductItem.createdAt.toISOString(),
          };
        }),
      }),
    );
